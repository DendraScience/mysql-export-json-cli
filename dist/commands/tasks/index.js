  ...
  return {
    help(p) {
      return style.commandHelp({
        ...
        synopsis: [{ cmd: '<command>', opts: '[<options>] [<args>]' }, {}, { cmd: 'multi', opts: '--filetmpl=<filetmpl> [--merge=ltr|rtl] --conn:<connectOption>[=<val>] ... (--exp:sqlString=<str> | --exp:tableName=<name>) [--exp:<exportOption>[=<val>] ...]' }, { cmd: 'ping', opts: '--conn:<connectOption>[=<val>] ...' }],
        groups: [{
          ...
          items: [{ opts: '--filetmpl=<filetmpl>', desc: 'Template string for naming JSON files (URI encoded), e.g. "{_id}.station.json"' }, { opts: '--merge=<direction>', desc: 'Merge exported rows with existing files (top-level fields only)' }]
        }, {
          header: 'Commands',
